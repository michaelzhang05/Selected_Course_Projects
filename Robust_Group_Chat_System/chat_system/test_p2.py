...
def init_containers():
    for i in range(1,NUM_SERVERS+1):
        cmd_str = "docker run --cap-add=NET_ADMIN --name {serv} --network {net} -- ip {ip} {img} {cmd} {args}".format(serv=server_name(i), net=NETWORK_NAME, img=IMAGE_NAME, ip=server_ip(i), cmd=RUN_COMMAND, args="-id {}".format(i))
        #subprocess.Popen(cmd_str, shell=True)
        subprocess.Popen(cmd_str, shell=True, stdout=subprocess.DEVNULL,
...
cmd_str1 = "docker exec {} tc qdisc change dev eth0 parent 1:{} handle {}:
netem loss {}".format(s1, id2+10, id2+10, loss)
# Set loss outgoing from container2 to container1
cmd_str2 = "docker exec {} tc qdisc change dev eth0 parent 1:{} handle {}:
netem loss {}".format(s2, id1+10, id1+10, loss)
# Run commands
