...
def init_containers():
    for i in range(1,NUM_SERVERS+1):
        cmd_str = "docker run --cap-add=NET_ADMIN -it --name {serv} --network {net} --ip {ip} {img} {cmd} {args}".format(serv=server_name(i), net=NETWORK_NAME,
        img=IMAGE_NAME, ip=server_ip(i), cmd=RUN_COMMAND, args="-id {}".format(i))
        #cmd_str = "docker run --cap-add=NET_ADMIN -it --name {serv} --network {net} --ip {ip} {img}".format(serv=server_name(i), net=NETWORK_NAME,
        #img=IMAGE_NAME, ip=server_ip(i)) 
